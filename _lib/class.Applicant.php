...
class Applicant {
	...
	public function getJobApplicationsById($id) {
		...
		$sql = 'SELECT UNIX_TIMESTAMP(a.created_on) as "created_on", a.status as "status", b.title as "job_title", b.salary as "job_salary", b.id as "job_id", b.company as "job_company" FROM '.DB_PREFIX.'job_applications a, '.DB_PREFIX.'jobs b WHERE a.job_id = b.id AND a.applicant_id = "' . $id . '"';
		$result = $db->query($sql);
		$final = array();
	...
	public function getCvDatabase($offset, $theme, $occupation, $location, $skills) {
		...
		$sql = 'SELECT id, fullname, occupation, email, phone, message, weblink, 
				...
				WHERE public_profile = 1 ' . $cond . ' ORDER BY UNIX_TIMESTAMP(last_activity) DESC limit ' .$offset . ', ' . APPLICATIONS_PER_PAGE;
