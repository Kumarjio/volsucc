...
<div class="row latest-jobs {if $job.spotlight == "1"}spotlight{/if}">
	...
	<div class="col-md-7 colx-xs-12">
		...
		<a href="{$BASE_URL}{$URL_JOB}/{$job.id}/{$job.url_title}" >
			<h3>{$job.title} {if $job.new_flag}<span class="new">{$translations.job_detail_section.new}</span>{/if} </h3>
		</a>
		...
		<ul>
			<li><li><span><i class="fa fa-map-marker" aria-hidden="true"></i></span>{$job.location_asci}</li>
			<li><li><span><i class="fa fa-calendar" aria-hidden="true"></i></span>{$job.created_on}</li>
		</ul>
	</div>
