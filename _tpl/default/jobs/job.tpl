			...
			<div class="container">
				<div class="row top-heading">
					<div class="col-md-12 col-xs-12">
						...
						<h1>{$job.title}</h1>
						<h2>{$translations.registration.go_home} > {$job.category_name} > {$profile.fullname}</h2>
					</div>
				</div>
				<div class="row top-heading">

					<div class="col-md-9 col-xs-12 details">
						<h3>{$job.title} {if $REMOTE_PORTAL == 'deactivated'}{$translations.website_general.in} {$job.location}{/if} {if $job.new_flag}<span class="new">{$translations.job_detail_section.new}</span>{/if}</h3>
						...
						<div class="col-md-3 col-xs-12 co-name">
							...
							{if $job.public_profile_flag == '1'}
								<a href="{$job.company_detail_url}"><img class="co-logo" src="/{$job.company_logo_path}" alt="company logo" /></a>
							{else}
