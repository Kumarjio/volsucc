...
if (intval($new_user) == 1) {
	...
	//create all users. But online those who ticked public = 1 will be listed for recruiters
	//saving all for database consistency, not spread info into 2 tables
	$cl = new Applicant();

	if (intval($public_profile) == 0) {
		if (!$cl->checkIfUserExists($apply_email)) {
		...
		switch ($status) {
			case 0:
				$APPLICANT_ID = $cl->getId();
				$cl->updateExistingApplicant($data, $APPLICANT_ID, false, $public_profile);
				break;
			case 1:
				$ERROR = "multiple_accounts_err";
				$smarty->assign('error_msg', $apply_email . ' ' . $translations['apply']['multiple_accounts_err']);
				break;
