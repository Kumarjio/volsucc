<?php
	...
	if ($job->Exists() && $job->GetActiveStatus() == 1)
	{
		...
		// if same IP in last hour has not had a click, record the view
		...
		$title = stripslashes($info['title']) . " " . $translations['website_general']['at'] . " " . stripslashes($info['company']);
		$keywords = stripslashes($info['title']) . ", " . $translations['website_general']['at'] . ", " . stripslashes($info['company']);
