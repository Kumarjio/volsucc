	...
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
				...
				*/
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
				{
					...
					$emp_class = new Employer();
					$class = new Company();

					//store the payment
					$emp_class->createPayment($data);
					...
					if ((intval($httpParsedResponseAr["AMT"]) != intval($ItemTotalPrice)) || PAYPAL_CURRENCY_CODE != $httpParsedResponseAr["CURRENCYCODE"])  {
						...
						$orders[] = array('desc' => $translations['invoice']['account_renewal_description'], 'price' => $_SESSION['PRICE_ORIG']);
						 ...
						 $emp_class->createInvoice($EMPLOYER_ID, BASE_URL . 'uploads/invoices/' . $invoice_filename . '.pdf');
					     
						// renew package for employer
						$emp_class->renewAccountPackage($PACKAGE_ID, $EMPLOYER_ID);
						 ...
						 unset($_SESSION['PACKAGE_ID']);unset($_SESSION['PRICE_ORIG']);
						 
						 $_SESSION['PAYPAL_RESULT_MESSAGE'] = $result_msg;

						 // redirect
						 redirect_to(BASE_URL . URL_DASHBOARD . '/' . URL_DASHBOARD_ACCOUNT_ORDER . '/' . URL_DASHBOARD_ACCOUNT_ORDER_COMPLETED);
					}
