				...
				'<div class="categoryWrapper">',
					'<div class="row-fluid">',
						'<div id="inputs" class="col-md-9 col-sm-12">',
							'<label><span class="gray">Name:</span><input id="a" class="light-text form-control minput" type="text" size="60" name="name[]" value="Например: Верстальщик" /></label>',
							'<label><span class="gray typesPadding">Title:</span><input id="b" class="light-text form-control minput" type="text" size="60" value="Например, лучшие вакансии верстальщик" name="title[]" /></label>',
							'<label><span class="gray typesPadding">Description:</span><input id="c" class="light-text form-control minput" type="text" value="Например, лучшие вакансии верстальщик" size="60" name="desc[]" /></label>',
							'<label><span class="gray">Keywords:</span><input id="d" class="light-text form-control minput" type="text" size="60" name="keys[]" value="Например: HTML, DHTML, CSS, XML, XHTML, JavaScript" /></label>',
							'<label><span class="gray typesPadding">URL:</span><input id="e" class="light-text form-control minput" type="text" size="60" name="url[]" value="Например: Верстальщик" /></label>',
						'</div>',
		...
		var mouseClick = function(e) { 
			...
			if ($(el).is('a.deleteCategory')) {
					...
					$.ajax({
						...
						success: function(responseText) {
								...
								if (json.result == '0'){
									...
									jobberBase.messages.add('Категория удалена');
									updateCategoriesList();
								}
				...
				$.ajax({
					...
					success: function() {
						jobberBase.messages.add('Категория сохранена');
						$(el).hide();
						updateCategoriesList();
					},
					complete: function(){
