		...
		<br />
			{section name=tmp loop=$settings_categories}
				<div class="settings-row">
					...
					<a class="right" href="{$BASE_URL_ADMIN}settings/{$settings_categories[tmp].var_name}/" title="Edit {$settings_categories[tmp].name}"><span class="fs13 fui-gear blueColor" ></span>&nbsp;</a>
			...
			<div class="settings-row">
				...
				<label class="settings"><a class="blue-font" href="{$BASE_URL_ADMIN}password/">Изменения пароля</a></label>
				<div class="light">УПРАВЛЕНИЕ ПОЛНОМОЧИЯМИ</div>
			</div>
	{/if}

	{if $settings_form != ''}
		{if $category_name !=  ''} <label class="settings">{$category_name}</label> <a href="/sjs-admin/settings"><div class="subheading">&larr;назад</div></a>{/if}
		{if $category_name|lower == 'invoicing'}<span style="font-size: 11px; opacity: 0.75;">To edit rest of the texts, change translations - "invoice" section</span>{/if}
...
{if $input_type == 'text_area'}
			...
			<div class="button-holder-admin" style="margin-top: 25px;">
				...
				<a href="{$BASE_URL_ADMIN}settings/" style="text-decoration: none;">
					<button type="button" class="btn btn-default btn-warning mbtn" name="button" id="button" >Отмена</button>
				</a>
